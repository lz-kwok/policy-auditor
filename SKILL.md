...
## 📂 待审计文档提取工具 (Document Text Extraction)

由于企业制度文件可能存储为多种文档格式（如 `.doc`、`.docx`、`.pdf`、`.xls`、`.xlsx`），Agent 或用户可以在审计前运行以下脚本，将文档内容统一提取为纯文本，以便于分析 and 审计：

* **提取脚本路径**：`[read_document.py](scripts/read_document.py)`
* **运行命令**：
  ```bash
  python scripts/read_document.py --file <待审计文件路径> [--output <输出文本文件路径>]
  ```
* **工作逻辑与库依赖**：
  * **`.docx` 格式**：依赖 `python-docx`，读取全部段落及表格数据，并自动对齐表格单元格。
  * **`.doc` 格式**：仅在 Windows 环境下支持，依赖 `pywin32` (win32com)，调用 MS Word 实例进行文本导出。
  * **`.pdf` 格式**：依次尝试 `pypdf` -> `pdfplumber` -> `PyMuPDF` (fitz) 作为降级机制提取文字。
  * **`.xlsx` 格式**：依赖 `openpyxl`，按 Sheet 逐行读取数据。
  * **`.xls` 格式**：依赖 `xlrd`（非 Windows 下必须），在 Windows 环境下支持通过 `pywin32` (win32com) 调用 MS Excel 实例进行读取。
...
1. **职能领域规则包 (Rulesets)**:
   ...
   - **工程建设与基建管理类**：文本包含“基建、工程建设、立项、设计变更、现场签证、隐蔽工程、竣工决算、承包商、造价审计、质保金”等 -> **加载** `[engineering.md](references/rulesets/engineering.md)`
...
1. **文本提取与规则加载**：对待审计的原始文档（如 `.doc`、`.docx`、`.pdf`、`.xls`、`.xlsx`）使用 `read_document.py` 脚本提取出纯文本内容。分析文本内容，识别其职能领域与行业背景，并使用 `view_file` 动态读取对应的规则包。
...
**加载规则库**：
- 核心底座：企业制度与流程审计核心规则
- 职能规则集：[未加载 / 采购与招标管理类规则集 / 人力资源管理类规则集 / 数智化建设与运营类规则集 / 工程建设与基建管理类规则集]
- 行业知识库：[未加载 / 生猪养殖与生物安全类行业规范]
...
* **条款修改对比**：【排版红线】为便于各端阅读与审查，所有 diff 代码块内的单行文本长度必须严格限制，严禁单行文本过长。必须合理进行**手动换行**，确保在最终渲染的报告中**绝对不能出现左右水平滑动条（不要滑动，注意换行）**。
```diff
- [修改前的原始条款（合理进行手动换行以防止产生滑动条）]
+ [修改后的合规且可执行条款（合理进行手动换行以防止产生滑动条）]
```
```
